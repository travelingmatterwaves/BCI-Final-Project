classdef (Sealed) IEEGDataset < IEEGObject
  ...
  methods(Access = private, Sealed = true)
    function populate_timeseries(obj) 
      obj.channels(obj.ts.size) = IEEGTimeseries();
      detailIterator = obj.ts.iterator;
      ix = 0;
      while detailIterator.hasNext
        ix = ix +1;
        obj.channels(ix) = IEEGTimeseries(detailIterator.next, obj);
        setparent(obj.channels(ix), obj);
      end        
    end
    ...
    function [values,x] = getdatablock(obj, startIndex, blockLength, chIdx)
      ...
      sampleRate = obj.channels(chIdx(1)).sampleRate;
      ...
      assert(min(chIdx) >= 1 && max(chIdx) <= length(obj.channels),...
        'Channel Indeces out of range.');
      
      %Get channelIds 
      revIDchID = java.util.ArrayList;
      for i = 1:length(chIdx)
        tsDetails = obj.channels(chIdx(i)).get_tsdetails;
      ...
      for iChannel = 1:length(chIdx)
        ...
        % scalefactor.
        if iChannel==1
          values = zeros(firstSeries.getSeriesLength, length(chIdx));
      ...
      if ~isnan(obj.filterOrder)
        ...
        % Remove offset added for filtering...
        if offset > 0
          offsetValues = round(((offset/1e6) * sampleRate));
      ...
      if ~isnan(obj.resampleRate)
        [values, x] = resampleData(obj, values, sampleRate);
  ...
  methods
    function obj = IEEGDataset(varargin)
      ...
      try        
        ...
        if strfind(ME.message,'TimeSeries.NoSuchDataSnapshot')
    ...
    function fs = get.sampleRate(obj)
      ...
      if isnan(obj.resampleRate)
        allSampleRate = [obj.channels.sampleRate];
  ...
  methods (Sealed = true)
      ...
      switch nargin
        case 3
          ...
          if ischar(chIdx)
            ...
            chIdx = 1:length(obj.channels);
          end
          
          allSampleRates = [obj.channels.sampleRate];
          ...
          startIndex = floor((1e6*(tIdx(1)-1))./allSampleRates(1));
          blockLength = (1e6*(tIdx(end)-tIdx(1)+1))./allSampleRates(1);
          
          
          assert(all(chIdx>=1)&& all(chIdx<=length(obj.channels)),...
            'Channel indeces out of range.');
    ...
    function setFilter(obj, n, Wn, type)
      ...
      assert(min(Wn) > 0 && max(Wn) < obj.channels(1).sampleRate/2,...
        'Cutoff freq must be between 0 and the nyquist-frequency of the data.');
            
      obj.filterOrder = n;
      obj.filterType = type;
      obj.filterWn = Wn /(obj.channels(1).sampleRate/2);  
    ...
    function h = filterSummary(obj)
      ...
      if any(strcmp('Signal Processing Toolbox', {v.Name}))
        ...
        set(h,'Fs',obj.channels(1).sampleRate);
      else
        h= [];
    ...
    function obj = rename(obj, newName)
      ...
      try
        ...
        if strfind(ME.message,'TimeSeries.AuthenticationFailure')
          throwAsCaller(MException('IEEGDATASET:RENAME',...
            'You are not authorized to change the name of this snapshot.'));
        elseif strfind(ME.message,'TimeSeries.DuplicateLabel')
          throwAsCaller(MException('IEEGDATASET:RENAME',...
  ...
  methods (Access = private)
    ...
    function [out, xq] = resampleData(obj, data, sf)
      % RESAMPLEDATA  Resamples the data (private method)
      
      % Method assumes all channels have same sampling rate!
      % This is enforced by the GETVALUES method.
      
      x = 1/sf * (1:size(data,1));
      xq = (1/obj.resampleRate) * (1:size(data,1));
      
      maxXq = find(xq > x(end),1);
      xq = xq(1:maxXq);
      
      % Interpolate
      out = interp1(x,data,xq,'linear');
