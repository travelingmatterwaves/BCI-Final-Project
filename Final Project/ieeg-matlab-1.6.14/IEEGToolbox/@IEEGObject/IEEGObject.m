classdef (Abstract) IEEGObject < handle
  ...
  methods (Sealed = true)
    ...
    function disp(obj)
      ...
      Link0 = sprintf(': <a href="matlab:help(''IEEGTimeseries'')">IEEGTimeseries</a>');
      Link1 = sprintf('<a href="matlab:help(''%s'')">%s</a>',class(obj),class(obj));
      Link2 = sprintf('<a href="matlab:methods(%s)">Methods</a>',class(obj));

      Link3 = sprintf('<a href="matlab:IEEGObject.openPortalSite()">ieeg.org</a>');
        ...
        if isvalid(obj)
          ...
          for i = 1:length(fieldn)
            ...
            if any(inSpecialProps)
              ...
              if specialProps(spIndex).size(1) ~= -1
